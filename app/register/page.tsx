    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-10 text-slate-100">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900 p-8 shadow-2xl shadow-cyan-500/10">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <select value={role} onChange={(e) => setRole(e.target.value)} className="w-full rounded-lg border border-white/15 bg-slate-800 px-3 py-2">
            <option value="student">Học sinh / Sinh viên</option>
            <option value="office">Nhân viên văn phòng</option>
            <option value="business">Doanh nghiệp</option>
          </select>
          {error ? <p className="text-sm text-rose-300">{error}</p> : null}
          <button type="submit" disabled={loading} className="w-full rounded-lg bg-cyan-400 px-4 py-2 font-semibold text-slate-950">{loading ? "Đang tạo..." : "Tạo tài khoản"}</button>
        </form>
        <p className="mt-5 text-sm text-slate-300">Đã có tài khoản? <Link href="/login" className="font-semibold text-cyan-300">Đăng nhập</Link></p>
      </div>
    </main>
  );
}
